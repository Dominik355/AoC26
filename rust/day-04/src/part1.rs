...
pub fn process(input: String) -> anyhow::Result<impl std::fmt::Debug> {
    ...
    for (i, &cell) in grid.iter().enumerate() {
        if cell {
            ...
            neighbors[0] = (row > 0, i - width_isize);
            neighbors[1] = (row < height - 1, i + width_isize);
            neighbors[2] = (col > 0, i - 1);
            neighbors[3] = (col < width - 1, i + 1);
            neighbors[4] = (row > 0 && col > 0, i - width_isize - 1);
            neighbors[5] = (row > 0 && col < width - 1, i - width_isize + 1);
            neighbors[6] = (row < height - 1 && col > 0, i + width_isize - 1);
            neighbors[7] = (row < height - 1 && col < width - 1, i + width_isize + 1);
